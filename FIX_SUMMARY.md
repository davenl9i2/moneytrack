...
---
## 5. 修正「今天」與「昨天」的時間錯亂 (Today vs Yesterday Context Fix)
- **問題**: 當使用者詢問「今天花費」時，Bot 正確撈出了今天的資料，但口語回應卻說「昨天你買了...」，因為 LLM 缺乏當下時間的參照，且過度遵守先前「非今日」的禁令。
- **修復**:
  - **程式碼更新 (`lib/groq.ts`)**: 在 Summary Prompt 中明確注入了 `Current Date` (台灣時間)。
  - **新增判斷規則**: 
    - 若 `Query Date` == `Current Date`，強制 Bot 說「今天」。
    - 若 `Query Date` == `Current Date - 1`，強制 Bot 說「昨天」。
  - 現在問「今天花多少」與「昨天花多少」，Bot 都能精準切換正確的時間用語。
